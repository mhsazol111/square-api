...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    // Function to call when an image src changes
    function onImageSrcChange(mutation) {
      var currentSrc = mutation.target.src;
      var newSrc = currentSrc.split('?')[0];

      if (currentSrc !== newSrc) {
        // console.log('Old Image src was:', currentSrc);
        // Updating the image src to the new URL without query parameters
        mutation.target.src = newSrc;
        // console.log('New Image src set to:', newSrc);
      }
    }
    ...
    var observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        if (mutation.type === 'attributes' && mutation.attributeName === 'src') {
          onImageSrcChange(mutation);
        }
      });
    });

    // Observer options - watch for attribute changes specifically to the 'src' attribute
    var config = { attributes: true, attributeFilter: ['src'] };

    setTimeout(function () {
      const $featuredMenuItems = $(
        '[data-block-purpose="featured-menu-items@^1.0.0"] .figure__image img'
      );
